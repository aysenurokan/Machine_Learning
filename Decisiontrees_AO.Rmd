...
```{r}
set.seed(41)
tree.rpart=rpart(switch_choice~. -id,data=df) #predict switching choice from all predictors 
...
table(pred.rpart,tsdf$switch_choice) #cross-tabulation table (observed values are in columns)
miss.rpart=1-mean(pred.rpart==tsdf$switch_choice, na.rm = T) #0-1 loss 
...
table(pred.rpartp,tsdf$switch_choice)
miss.rpartp=1-mean(pred.rpartp==tsdf$switch_choice, na.rm = T)
miss.rpartp
```
...
```{r}
#bag1=randomForest(switch_choice~.-id,data=df,mtry=7,ntree=500) #bagging model; `mtry` function argument controls the number of predictors considered every time the tree makes a split. If the `mtry` values equals the number of predictors, then the randomForest function does bagging.

#Error above due to missing value; removing and fitting the model again
dfn <- na.omit(df)
tsdfn <- na.omit(tsdf)
bag1 <- randomForest(switch_choice ~ ., data = dfn, mtry = 7, ntree = 500)

bagpred=predict(bag1,tsdfn,type='response') #obtain predicted values from testing dataset 
bag.table=table(bagpred,tsdfn$switch_choice) #cross-tabulation (rows are predicted, columns are observed 
bag.table

1-(mean(bagpred==tsdfn$switch_choice))
...
```{r}
# Predicting switch_choice with random forest on the training dataset
rf1=randomForest(switch_choice~.-id,data=dfn,mtry=3,ntree=400) #use 3 vars at a time (3 random predictors as candidates to split on), 400 trees grown in bootstrapped datasets. 
#obtain the predicted values in the testing dataset
rfpred=predict(rf1,tsdfn,type='response')
# cross-tabulation of the classes
rf.table=table(rfpred,tsdfn$switch_choice)
#the variable importance measures.
rf.table
1-(mean(rfpred==tsdfn$switch_choice))
varImpPlot(rf1)
```
...
crf1 <- cforest(switch_choice ~ . - id, data = dfn, controls = cforest_unbiased(ntree = 400, mtry = 3))
# Make predictions on the testing dataset
crfpred <- predict(crf1, newdata = tsdfn, type = "response")
crf.table=table(crfpred,tsdfn$switch_choice)
crf.table
1-(mean(crfpred==tsdfn$switch_choice))
vi=varimp(crf1)
...
#G#row a random forest with randomForest() to predict switch_choice, and try 5 tuning parameters and the previously-specified cross-validation routine.
```{r}
set.seed(2000)
#Fit Model 
train.rf=train(switch_choice~.,data=dfn,method='rf',tuneLength=5,trControl=trctrl) 
...
set.seed(2000)
train.crf=train(switch_choice~.,data=dfn,method='cforest',tuneLength=5,trControl=trctrl)
train.crf
plot(train.crf)
